...
const signUpButton      = document.getElementById('to-login');
const loginTakenError   = document.getElementById('username-taken-error');
var   loginAlreadyTaken = false;
...
async function sign_up(username, password) {
  ...
  const response = await fetch('http://127.0.0.1:6789/register', {
    method: 'POST',
    headers: {
...
// Check login is taken
function setLoginAlreadyTaken(set) {
  loginAlreadyTaken = set;
  if (set) {
      loginTakenError.classList.add('active');
  } else {
      loginTakenError.classList.remove('active');
  }
}

async function checkLoginAlreadyTaken(login) {
  if (login === '') {
    setLoginAlreadyTaken(false);
    return;
  }
  const response = await fetch(`http://127.0.0.1:6789/user/exists?username=${login}`, {
    headers: {
      'accept': 'application/json'
    }
  });
...
// Send button
applyButton.onclick = () => {
    if (loginAlreadyTaken) return;
    let result = getCheckValues(loginInput, passwordInput);
    if (result == undefined) return;
    sign_up(result.login, result.password);
}
...
function loginCheckValidate(object) {
    ...
    checkLoginAlreadyTaken(object.value);
    return result;
}
